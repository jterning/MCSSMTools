...
***********************************************************************
      ...
      DOUBLE PRECISION DMA,HMIN,HMAX,MHIG(5),BDCMS(5),BCMS,BDCMST(5)
      DOUBLE PRECISION CMSBD
...
*     Bound on a single Higgs
      DO I=1,5
        BDCMS(I)=BCMS(MHIG(I))*CD(I)**2*BRLL(I)
      ENDDO

* Sum over Higgses < 20 GeV apart
      DO I=1,5
        BDCMST(I)=BDCMS(I)
        DO J=1,5
          IF((DABS(MHIG(J)-MHIG(I)).LE.10.D0).AND.(J.NE.I))
     .      BDCMST(I)=BDCMST(I)+BDCMS(J)
        ENDDO
      ENDDO
...
* Compare to LHC bounds
      PROB(45)=0D0
      DO I=1,5
        IF(BDCMST(I).GE.CMSBD(MHIG(I)))
     .    PROB(45)=PROB(45)+(BDCMST(I)-CMSBD(MHIG(I)))/2
      ENDDO
...
* LHC constraints on H/A -> 2tau, divided by (2*CD(I)**2*BR(2tau))

      DOUBLE PRECISION FUNCTION BCMS(MX)
      IMPLICIT NONE
      INTEGER I
      DOUBLE PRECISION MX,B(10)
      
      DATA B/0.992D0,.603D0,.258D0,.226D0,.153D0,.0882D0,.0518D0,
     .    .0320D0,.0112D0,.004555D0/
     
      BCMS=0D0
      IF((MX.GE.88.D0).AND.(MX.LE.90D0))
     .  BCMS=B(1)
      IF((MX.GE.90D0).AND.(MX.LE.100D0))
     .  BCMS=B(1)+(MX-90D0)*(B(2)-B(1))/10D0
      IF((MX.GE.100D0).AND.(MX.LE.120D0))
     .  BCMS=B(2)+(MX-100D0)*(B(3)-B(2))/20D0
      IF((MX.GE.120D0).AND.(MX.LE.130D0))
     .  BCMS=B(3)+(MX-120D0)*(B(4)-B(3))/10D0
      IF((MX.GE.130D0).AND.(MX.LE.140D0))
     .  BCMS=B(4)+(MX-130D0)*(B(5)-B(4))/10D0
      DO I=1,3
        IF((MX.GE.140D0+(I-1)*20D0).AND.(MX.LE.140D0+I*20D0))
     .    BCMS=B(I+4)+(MX-(140D0+(I-1)*20D0))*(B(I+5)-B(I+4))/20D0
      ENDDO
      DO I=1,2
        IF((MX.GE.200D0+(I-1)*50D0).AND.(MX.LE.200D0+I*50D0))
     .    BCMS=B(I+7)+(MX-(200D0+(I-1)*50D0))*(B(I+8)-B(I+7))/50D0
      ENDDO
      IF((MX.GE.300D0).AND.(MX.LE.302.D0))
     .  BCMS=B(10)
      END


* LHC constraints on sigma(pp->H/A)*BR(H/A->2tau)

      DOUBLE PRECISION FUNCTION CMSBD(MX)
      IMPLICIT NONE
      INTEGER I
      DOUBLE PRECISION MX,B(14)
      
* OLD:      DATA B/147.74D0,112.3D0,39.61D0,25.4D0,18.2D0,11.37D0,9.78D0,
*     .    8.71D0,5.77D0,4.36D0/
* NEW:
       DATA B/14.1D0,8.D0,4.5D0,4.1D0,3.8D0,3.1D0,2.3D0,2.4D0,1.7D0,
     .    1.2D0,0.9D0,0.6D0,0.42D0,0.34D0/ 
      CMSBD=1.D90
      IF((MX.GE.90D0).AND.(MX.LE.100D0))
     .  CMSBD=B(1)+(MX-90D0)*(B(2)-B(1))/10D0
      IF((MX.GE.100D0).AND.(MX.LE.120D0))
     .  CMSBD=B(2)+(MX-100D0)*(B(3)-B(2))/20D0
      IF((MX.GE.120D0).AND.(MX.LE.130D0))
     .  CMSBD=B(3)+(MX-120D0)*(B(4)-B(3))/10D0
      IF((MX.GE.130D0).AND.(MX.LE.140D0))
     .  CMSBD=B(4)+(MX-130D0)*(B(5)-B(4))/10D0
      DO I=1,3
        IF((MX.GE.140D0+(I-1)*20D0).AND.(MX.LE.140D0+I*20D0))
     .    CMSBD=B(I+4)+(MX-(140D0+(I-1)*20D0))*(B(I+5)-B(I+4))/20D0
      ENDDO
      DO I=1,6
        IF((MX.GE.200D0+(I-1)*50D0).AND.(MX.LE.200D0+I*50D0))
     .    CMSBD=B(I+7)+(MX-(200D0+(I-1)*50D0))*(B(I+8)-B(I+7))/50D0
      ENDDO
