...
* [13] E. Barberio et al. [Heavy Flavor Averaging Group (HFAG)],
*     ' Averages of b-hadron properties at the end of 2005'
*     arXiv:hep-ex/0603003, and www.slac.stanford.edu/xorg/hfag.
*
...
* [20] Combined LHCb and CMS constraints on Bs -> tau+ tau-
     ...
     .       BRBMUMUexpMax,BRBTAUNUexpmin,BRBTAUNUexpMax,
     .       BRSGexpmin,BRSGexpMax
...
*      Delta Ms from [14], 2 sigma bounds:
*      17.53ps-1 < DMs=17.77ps-1 < 18.01ps-1
      DMsexpmin=17.53d0
      DMsexpMax=18.01d0

*      From [20]: BR(Bs->mu+mu-) < 1.1 10^-8 (95% C.L.)
      BRBMUMUexpmax=1.1d-8
      
*       BR(B+ -> tau+ nu) from [19], 2 sigma bounds:
*      0.89 10^-4 < BR(B+ -> tau+ nu)=1.67 10^-4 < 2.45 10^-4
      BRBTAUNUexpMax=2.45d-4
      BRBTAUNUexpmin=0.89d-4

*       BR(B -> Xs gamma) from [13], 2 sigma bounds:
*   3.03 10^-4 < BR(B -> Xs gamma)=3.52 10^-4 < 4.01 10^-4
      BRSGexpmin=3.03d-4
      BRSGexpMax=4.01d-4
...
*      write(0,*) "DMd",aux,DSM,DH,Dchid,DDP
...
*       (Recall: 2 sigma bounds: 17.53ps-1 < DMs < 18.01ps-1)
...
*    (Recall 95% C.L.: BR(Bs->mu+mu-) < 1.1 10^-8)

      prob(35)=0d0

      IF(BRBMUMUmin.GE.BRBMUMUexpMax)
     .     PROB(35)=(BRBMUMUmin-BRBMUMUexpMax)*1.d8
...
*       * 2 sigma bounds (source [13])
*   3.07 10^-4 < BR(B -> Xs gamma)=3.55 10^-4 < 4.07 10^-4

      prob(32)=0d0

      IF(BRSG+DBRSGmin.GE.BRSGexpMax)
     .     PROB(32)=(BRSG+DBRSGmin-BRSGexpMax)*1.D4
      IF(BRSG+DBRSGmax.LE.BRSGexpmin)
     .     PROB(32)=(BRSG+DBRSGmax-BRSGexpmin)*1.D4
...
*   0.60 10^-6 < BR(B -> Xs l+l-)=1.60 10^-6 < 2.60 10^-6
     ...
     .     PROB(40)=(BRBSllmin-2.6d-6)*1.D6
      IF(BRBSllmax.LE.0.6d-6)
     .     PROB(40)=(BRBSllmax-0.6d-6)*1.D6
...
*      prob(40)=0d0
      IF(PROB(40).eq.0d0)then
      IF(BRBSllmin.GE.6.8d-7)
     .     PROB(40)=(BRBSllmin-6.8d-7)*1.D7
      IF(BRBSllmax.LE.2.0d-7)
     .     PROB(40)=(BRBSllmax-2.0d-7)*1.D7
      ENDIF
