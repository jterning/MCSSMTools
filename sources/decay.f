...
* New July 2010:
      HQCD(X)=(4d0/3*HQCDM(BETA(X))
     .   +2*(4d0/3-DLOG(X))*(1-10*X)/(1-4*X))*ASH/PI
     .   + (29.14671d0 + RATCOUP*(1.570d0 - 2*DLOG(HIGTOP)/3
     .   + DLOG(X)**2/9))*(ASH/PI)**2
     .   +(164.14d0 - 25.77d0*5 + 0.259d0*5**2)*(ASH/PI)**3
      AQCD(X)=(4d0/3*AQCDM(BETA(X))
     .   +2*(4d0/3-DLOG(X))*(1-6*X)/(1-4*X))*ASH/PI
     .   + (29.14671d0 + RATCOUP*(23/6d0 - DLOG(HIGTOP)
     .   + DLOG(X)**2/6))*(ASH/PI)**2
     .   + (164.14d0 - 25.77d0*5 + 0.259d0*5**2)*(ASH/PI)**3
* End New
      QCDH(X)= 1d0+HQCD(X)
...
*   them by the SM couplings in order to obtain reduced couplings
     ...
     .  + CV(I)*CXW + GHCC(I)/(2d0*CMASS**2*DSQRT(2d0*SQR2*GF))*CXHC
...
*   h -> bb
       ...
       RATCOUP= CU(I)/CD(I)
* Allow H->bb only for MH > 2*M_B = 10.6 GeV
...
*  h -> Z gamma
     ...
     .     * CI1(CTHC,CLH)*GHCC(I)/(DSQRT(2d0*SQR2*GF))
...
*   h -> hh
       ...
       IF(I.EQ.2)THEN
        ...
        ELSE
         ...
         HTOT= HTOT+HHH(1)
*    	 write(0,*)"HHH(1)",RH,SMASS(1),MH,CH,HHH(1),HTOT
        ENDIF
       ENDIF
...
*  Branching ratios
*       write(0,*)"WIDTH(",I,")",HJJ,HMM,HLL,HSS,HCC,HBB,HTT,HWW,HZZ
*       write(0,*)"WIDTH(",I,") cont. ",HGG,HZG,HTOT,STOT
...
*   a -> bb
       ...
       RATCOUP= CU(I+3)/CD(I+3)
* Allow A->bb only for MA > 2*M_B = 10.6 GeV
