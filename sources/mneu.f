...
*      DOUBLE PRECISION QSTSB,B0,B1
      ...
      DOUBLE PRECISION Tad,f,mSing2,majS,cw,sw,cb,sb,yy,vv
...
*     .       12d0*(HTQ**2+HBQ**2)*B1(MU2,0d0,QSTSB,Q2)
*     .       +(3d0*G2+G1)*(B1(MU2,M22,MA2,Q2)+B1(MU2,M22,MZ2,Q2)
*     .       +2d0*B1(MU2,MU2,MZ2,Q2)-4d0*B0(MU2,MU2,MZ2,Q2))))

* M1 incl. rad. corrs.:

*     M1=M1*(1d0-G1/(16d0*PI**2)*(11d0*B1(M12,0d0,MSQ,Q2)
*     .     +9d0* B1(M12,0d0,MSL,Q2)
*     .     +SIN2B*MU/M1*(B0(M12,MU2,MA2,Q2)-B0(M12,MU2,MZ2,Q2))
*     .     +B1(M12,MU2,MA2,Q2)+B1(M12,MU2,MZ2,Q2)))

* M2 incl. rad. corrs.:

*      M2=M2*(1d0-G2/(16d0*PI**2)*(9d0*B1(M22,0d0,MSQ,Q2)
*     .     +3d0* B1(M22,0d0,MSL,Q2)
*     .     +SIN2B*MU/M2*(B0(M22,MU2,MA2,Q2)-B0(M22,MU2,MZ2,Q2))
*     .     +B1(M22,MU2,MA2,Q2)+B1(M22,MU2,MZ2,Q2)
*     .     -8d0*B0(M22,M22,MW2,Q2)+4d0*B1(M22,M22,MW2,Q2)))
...
*******************************************************************
* Neutralino Mass matrix for minimal composite supersymmetric standard model
*
      yy=LQ
      vv=4 * MZ**2/(g1q+g2q)
      cw=DSQRT(g2q/(g1q+g2q))
      sw=DSQRT(1-cw**2)
      ...
      NEU(3,3)=0d0
      NEU(3,4)=-mu
      NEU(3,5)=-sb*DSQRT(vv)*yy
      NEU(4,4)=0d0
      NEU(4,5)=-cb*DSQRT(vv)*yy
      NEU(5,5)=majS
      NEU(2,1)=NEU(1,2)
      NEU(3,1)=NEU(1,3)
      NEU(4,1)=NEU(1,4)
      ...
      write(0,*)"Neutralino Mass Matrix"
      DO I= 1,5
         write(0,*)NEU(I,1),NEU(I,2),NEU(I,3),NEU(I,4),NEU(I,5)
      ENDDO     
