...
************************************************************************
      ...
      INTEGER NFAIL(NFL),IFAIL,I,TOT,ITOT,IDIM
      INTEGER ITER,ITERMU,MUFLAG,SFFLAG,Q2FIX,IM
      INTEGER AKFLAG,ALFLAG,MHDFLAG,MHUFLAG,M1FLAG
      INTEGER M2FLAG,M3FLAG,MAFLAG,OMGFLAG,NMSFLAG,NMSCAN
      INTEGER NM0,NM12,NTB,NA0,NL,NAK,NAL,NMHD,NMHU
      INTEGER NM1,NM2,NM3,NTOT
      INTEGER IM0,IM12,ITB,IA0,IL,IAK,IAL,IMHD,IMHU
      INTEGER IM1,IM2,IM3
     ...
     . XIFINP,XISINP,MUPINP,MSPINP,MSINP,M3HINP
      COMMON/STEPS/NTOT,NM0,NM12,NTB,NA0,NM1,NM2,NM3,NMHD,NMHU,
     . NL,NAL,NAK
...
*   Beginning of the scan
      ...
      ELSE
       IF(NMHD.EQ.1)THEN
        MHDINP=MHDMIN
       ELSE
        MHDINP=MHDMIN+(MHDMAX-MHDMIN)*DFLOAT(IMHD-1)/DFLOAT(NMHD-1)
       ENDIF
      ENDIF
      ...
      ELSE
       IF(NMHU.EQ.1)THEN
        MHUINP=MHUMIN
       ELSE
        MHUINP=MHUMIN+(MHUMAX-MHUMIN)*DFLOAT(IMHU-1)/DFLOAT(NMHU-1)
       ENDIF
      ENDIF
...
*******************************************************************
      ...
      INTEGER NTOT,NM0,NM12,NTB,NA0,NM1,NM2,NM3,NMHD,NMHU
      INTEGER NL,NAL,NAK
...
*   Number of points
      ...
      IF(MHDFLAG.EQ.0 .AND. NMHD.NE.1)THEN
       WRITE(0,1)"NMHD CANNOT BE GIVEN IN BLOCK STEPS"
       ERR=1
      ENDIF

      IF(MHUFLAG.EQ.0 .AND. NMHU.NE.1)THEN
      ...
      IF(NM0.LE.0 .OR. NM12.LE.0 .OR. NTB.LE.0 .OR. NA0.LE.0
     . .OR. NM1.LE.0 .OR. NM2.LE.0 .OR. NM3.LE.0 .OR. NMHD.LE.0
     . .OR. NMHU.LE.0 .OR. NL.LE.0 .OR. NAL.LE.0 .OR. NAK.LE.0)THEN
...
*   Total number of points
      ...
      NTOT=NM0*NM12*NTB*NA0*NM1*NM2*NM3*NMHD*NMHU
     .   *NL*NAL*NAK
